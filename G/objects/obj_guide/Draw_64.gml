		...
		case(0): //enemies
			...
			for (var row = 0; row < rows; row ++){
				for(var col = 0; col < cols; col ++){
					...
					if(discovered_enemies[enemy_index].discovered){
			...
			if(discovered_enemies[selection_index].discovered){
				draw_sprite(discovered_enemies[selection_index].sprite, frame, origin_x + 135, origin_y - 90);
				
				draw_set_halign(fa_left);
				draw_set_font(fnt_small);
				draw_text(origin_x + 80, origin_y - 60, discovered_enemies[selection_index].name);
				draw_text(origin_x + 80, origin_y + 80, "DROPS:");
				
				draw_set_font(fnt_paragraph);
				draw_set_color(c_white);
				draw_set_valign(fa_top);
				draw_text_ext(origin_x + 80, origin_y - 58, discovered_enemies[selection_index].description, 10, 105);
				draw_set_color(#dfdf45);
				draw_set_valign(fa_bottom);
				
				for(var i = 0; i < array_length(discovered_enemies[selection_index].drops); i++){
					var sprite = discovered_enemies[selection_index].drops[i]
		...
		case(1): //Props
			...
			for (var row = 0; row < rows; row ++){
				for(var col = 0; col < cols; col ++){
					...
					if(props[prop_index].discovered){
						draw_sprite(spr_icon_prop, prop_index + 1, grid_x + (col * cell_w), grid_y + (row * cell_h));
					}
					else{
						draw_sprite(spr_icon_prop, 7, grid_x + (col * cell_w), grid_y + (row * cell_h));
					}
				}
			}
			...
			selection_index = sel_grid_x + (sel_grid_y * 9)
			if(props[selection_index].discovered){
				
				var prop_spr = props[selection_index].sprite
				draw_sprite(prop_spr, frame, origin_x + 135 - (sprite_get_width(prop_spr) / 2) + sprite_get_xoffset(prop_spr), origin_y - 90);
				
				draw_set_halign(fa_left);
				draw_set_font(fnt_small);
				draw_text(origin_x + 80, origin_y - 60, props[selection_index].name);
				draw_text(origin_x + 80, origin_y + 80, "COST:");
				
				draw_set_font(fnt_paragraph);
				draw_set_color(c_white);
				draw_set_valign(fa_top);
				draw_text_ext(origin_x + 80, origin_y - 58, props[selection_index].description, 10, 105);
				draw_set_color(#dfdf45);
				draw_set_valign(fa_bottom);
				
				for(var i = 0; i < array_length(props[selection_index].cost); i++){
					var sprite = props[selection_index].cost[i]
					draw_sprite(sprite, frame, origin_x + 80 + sprite_get_xoffset(sprite) + (i * 28), origin_y + 86 + sprite_get_yoffset(sprite));
				}
			}
			else{
